...
import { HeadMeta, CustomOption, Filtering, Card, Footer, Nav } from '/components';
import {
	filterCityAtom,
	filterCountryAtom,
	filterDistanceAtom,
...
const Home = ({ cities, services, settings, airports }) => {
	...
	useEffect(() => {
		if (!query.country) {
			...
			if (manualCountry) {
				...
				router.push(`/clinics?country=${manualCountry}`);
			} else {
				setFilterCountry(countryOptions.filter((option) => option.value === defaultCountry)[0].value);
				router.push(`/clinics?country=${defaultCountry}`);
			}
		}
		...
		<div>
			...
			<section className="min-h-[calc(100vh-(189px*2))]">
				<main className="container mx-auto px-6 md:px-16 relative">
					<HeadMeta
						title="Ordinacije"
						link={`${process.env.BASE_URL}/clinics`}
					...
					<section className="grid grid-cols-1 xl:grid-cols-4 xl:gap-5 pb-6 xl:py-6">
						<div className="flex flex-col">
							...
							<div className="flex flex-col items-center lg:items-start">
								<h2 className="text-[20px] font-[700] border-b border-[#818181] pb-4">{t('sort.text-1')}</h2>
						...
						<div className="col-span-3" ref={scrollRef}>
							<div className="lg:px-6 lg:pt-6 mt-5 md:mt-0 text-center md:text-left">
								<h1 className="text-[20px] md:text-[40px] text-[#21231E] font-[700] h-[60px]">{t('main.text-2')}</h1>
								{filteredClinics?.filter((clinic) => clinic?.user?.active)?.length === 0 && (
									<span className="text-primaryColor font-[600] text-[18px] md:text-[24px]">{t('main.text-3')}</span>
								)}
							</div>
							<div
								className={`lg:p-6 bg-white/[0.57] backdrop-blur-[16.5px] rounded-[8px] col-span-3 grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5 lg:mt-0`}
							>
